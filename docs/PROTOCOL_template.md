...
Set **from level** to current level -1, **channel** to 0, **to level** to 2250, **time** 7.5 (seconds) and execute fade command
    ...
    +I2CT=ADDRESS,REGISTER_FROM_LEVEL,w-1,00,w2250,7.5,COMMAND_FADE (RAW_CMD:x)

Set **channel** to all channels (0xff = -1), **to level** to 1533, **time** 7.5 and execute fade

    +I2CT=ADDRESS,REGISTER_CHANNEL,b-1,w1533,7.5,COMMAND_FADE (RAW_CMD:x)
...
Set **channel** to 1, **to level** to 777 and execute set level command
    ...
    +I2CT=ADDRESS,REGISTER_CHANNEL,01,w777 (RAW_CMD:+I2CT=17,03,01,09,03)
...
To send the command in one transmission, the time can be filled with any data
    ...
    +I2CT=ADDRESS,REGISTER_CHANNEL,01,w777,0.0,COMMAND_SET_LEVEL (RAW_CMD:+I2CT=17,03,01,09,03,00,00,00,00,10)
...
Response (0x02, 2 seconds)
    ...
    +I2CT=ADDRESS,02 (RAW_CMD:+I2CT=17,02)

Set temperature check interval/metrics reporting to 30 seconds

    +I2CT=ADDRESS,REGISTER_TEMP_CHECK_INT,b30 (RAW_CMD:+I2CT=17,29,1e)
...
Prints the cubic interpolation tables to the serial port. The first byte is the channel, -1 for all
    ...
    +I2CT=ADDRESS,REGISTER_COMMAND,COMMAND_PRINT_CUBIC_INT,b-1 (RAW_CMD:+I2CT=17,0a,57,ff)
...
`<level:int16>,<number of levels:uint8>,<x1:uint8>,<x2>,...,<y1:int16>,<y2>,...`
    ...
    +I2CT=ADDRESS,REGISTER_COMMAND,DIMMER_COMMAND_GET_CUBIC_INT,w10,b11,00,02,07,w0,w2500,w8192 (RAW_CMD:x)
    +I2CR=ADDRESS,b22 (RAW_CMD:x)
...
Print the entire configration on the serial port as backup
    ...
    +I2CT=ADDRESS,REGISTER_COMMAND,COMMAND_DUMP_CFG (RAW_CMD:+I2CT=17,0a,91)
...
Set zero crossing delay to -31 ticks or -15.5µs and print setting
    ...
    +I2CT=ADDRESS,REGISTER_COMMAND,COMMAND_SET_ZC,w-31 (RAW_CMD:x)
    +I2CT=ADDRESS,REGISTER_COMMAND,COMMAND_SET_ZC,-15.5 (RAW_CMD:x)
